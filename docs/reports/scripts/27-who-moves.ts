...
// 1.00 と 0.90 の間に崖が出たので、整数ちょうどが特別扱いになっていないかを疑う。
//
// `quantize` は `frac > 0 && rng.chance(frac)` なので、**v=1.00 だけが乱数を引かず
// 常に半径1**になる。0.99 は99%が半径1で1%が盲目。挙動としてはほぼ同じはずだが、
// 崖がちょうど整数にあるなら 24 が高い丘で見つけた格子の話の再来になる。
//
//   0.95・0.99 でも共存する → 崖は 0.90〜0.95 にあり、生態の話
//   0.99 で低い側が消える   → 崖は整数ちょうど。frac>0 なら何でも壊れる＝格子の話
//
// 1.05・1.10 は対照。1より上は半径1と2の混合で、どちらも「見る規則」なので
// 崩れないはず。ここが崩れたら見立てが根本から違う。
header('崖は整数ちょうどにあるのか（低い側は0固定・8シード・30000歩）');
for (const vHi of [1.1, 1.05, 0.99, 0.95]) {
  await row(`低0.00 / 高${vHi.toFixed(2)}`, { vLo: 0, vHi, mutate: 'none' });
}
