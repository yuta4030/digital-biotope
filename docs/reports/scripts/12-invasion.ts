...
// ---------------------------------------------------------------------------
header('谷で入れたほうが通るか（有利 0.57 / 投入時の在来個体数で分ける）');

/**
 * 走行ごとの平均に対する比で束ねる。平均の違う世界どうしでも並べられるようにするため。
 *
 * 刻みが広いのは在来の振れ幅が大きいから。基本構成は平均678に対して
 * 199〜1598（変動係数0.5）まで振れるので、比は0.3〜2.3の範囲に散る。
 */
byResidentBin(results[2], [0, 0.5, 0.75, 1.0, 1.25, 1.5]);
